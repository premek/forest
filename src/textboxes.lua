  ...
  draw = function(self, cam)
    for _,box in ipairs(boxes) do
      if box.time and box.time > 0 then
        ...
        local y = box.pos and (box.pos.y - h - 5) or 10 -- FIXME camera vs world cords
        local bx, by = 4, 2
        love.graphics.setColor(res.palette[4][1], res.palette[4][2], res.palette[4][3], alpha)
        love.graphics.setLineWidth(2)
        love.graphics.rectangle("line", x-bx, y-by-1, w+2*bx, h+2*by)
        love.graphics.setColor(res.palette[1][1], res.palette[1][2], res.palette[1][3], alpha)
